...
class TwitterClient: BDBOAuth1SessionManager {
    ...
    func userTimelineWithParams(params: NSDictionary?, user: Int?, completion: (tweets: [Tweet]?, error: NSError?) -> ()) {
        GET("1.1/statuses/user_timeline.json?user_id=\(String(user!))", parameters: params, progress: nil, success: { (operation: NSURLSessionDataTask!, response: AnyObject?) -> Void in
